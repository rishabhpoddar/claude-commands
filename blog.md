...
5. Now go through each blog post title, and create a draft blog post for each of them. The draft blog post should be a markdown file in the `/Users/rishabhpoddar/Desktop/trythisapp/work_log/blogs/drafts` directory. The file name should be the blog post title. You can refer to the associated log files to get the content for it.
